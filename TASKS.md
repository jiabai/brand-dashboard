...
- [x] Phase 8.2 更新核心架构文档和 README。✅ 验证条件：`docs/ARCHITECTURE.md`、`docs/DESIGN.md`、`docs/SECURITY.md`、README 与新架构一致。
...
- [x] Phase 7.2 接入真实情感分析数据。✅ 验证条件：正式情感页面不再依赖纯 mock 统计，真实无数据时展示明确空状态。
- [x] Phase 7.1 新增问答快照页。✅ 验证条件：可按品牌、平台、关键词、情绪、是否引用过滤原始回答，页面构建通过。
- [x] Phase 6.4 前端展示数据新鲜度和覆盖率。✅ 验证条件：dashboard 可显示指标生成时间、采集覆盖率、分析完整性，空状态不误导用户。
