...
import { fetchAnswerSnapshots } from '../dashboard.js';
...
test('fetchAnswerSnapshots serializes dashboard filters', async () => {
  writeAuthSession({
  ...
  globalThis.fetch = async (url, options) => {
    request = { url, options };
    return jsonResponse({ status: 'success', data: [], metadata: {} });
  };

  await fetchAnswerSnapshots({
    tenantKey: 'tenant_a',
    jobId: 'job_a',
  ...
  assert.equal(parsed.pathname, '/api/v1/dashboard/answer-snapshots');
  ...
  assert.equal(parsed.searchParams.get('sentiment'), 'positive');
  assert.equal(parsed.searchParams.get('has_reference'), 'true');
