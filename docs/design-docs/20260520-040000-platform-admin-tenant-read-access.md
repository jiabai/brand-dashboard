...
> 日期：2026-05-20，2026-05-21 修订
>
> 状态：已实现
>
> 范围：平台管理员读取所有 active 租户 dashboard 数据；不包含写操作、审计表和平台管理员表迁移。

## 1. 需求摘要

平台管理员需要从平台后台进入任意客户租户 dashboard 做排障和交付支持。该能力必须满足：

- 只读：只覆盖 dashboard 查询接口。
...
### 方案 B：Dashboard 路由使用平台只读旁路依赖

优点：平台域和租户成员域保持分离；只读能力可限制在 dashboard 路由组；对现有 repository 的 `tenant_key` 过滤无影响。
...
Dashboard 路由组从 `Depends(get_current_tenant)` 改为只读依赖。写接口不改。

## 4. 数据流

```mermaid
flowchart TD
  A["Dashboard API Request"] --> B["get_current_user"]
  ...
  E --> J["Dashboard service queries with tenant_key"]
  I --> J
```
...
- `web/src/components/platform/PlatformTenantsPage.jsx`
  - 租户行增加“任务状态”按钮。
  - 跳转 `/tasks/<tenantKey>/status`，避免用前端 `.env` 的默认 job 拼接 dashboard URL。
...
- 平台只读旁路只允许 `GET /api/v1/dashboard/*`。
...
**决策**：平台管理员获得 dashboard 级全租户只读旁路，不通过 `user_tenants` 表达。
