...
- 报告生成：报告 API 只能读取当前租户、当前项目下的指标快照和告警事件，不能接受客户端传入任意指标 JSON 作为可信数据。
- 数据质量：数据质量 API 必须使用 `tenant_key + project_id` 查询失败采集、过期分析和快照覆盖率，不能按裸 `project_id` 聚合。
- 速率限制：登录、邀请码验证、员工注册、激活、报告生成和重算类接口上线前必须纳入限流计划。
...
- 分析血缘：事实表、指标快照、告警和报告必须保留 `analysis_run_id`、`collection_job_id` 或项目血缘，便于审计和重算。
