...
| 422 参数校验错误未纳入业务信封 | `RequestValidationError` 仍输出 FastAPI 默认 `{"detail":[...]}`，与统一错误信封不一致；前端靠 `detail` 回退兜底，数组拼接可读性差 | `docs/design-docs/20260610-unified-error-envelope.md` | 为 `RequestValidationError` 注册信封 handler（message 汇总字段级错误），前端与测试同步断言新形状 |
