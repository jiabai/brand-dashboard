...
|------|----------------|--------|-------------------|
| 历史重复数据风险尚未在目标数据库完成审计 | Phase 2.1 已提供只读检查脚本，但本地 `api/.env` 指向的 SQLite 文件缺少业务表，不能代表线上或 dump 的真实重复风险。 | `api/scripts/check_duplicate_analysis_rows.py`, `docs/exec-plans/active/20260606-brand-monitoring-system-refactor.md` | 连接目标 MySQL 或导入 `data/geo_dump_20260509_135617.sql` 后运行 Phase 2.1 脚本；清理所有阻塞迁移的重复/碰撞记录，并把审计结果记录到 ExecPlan 或 changelog。 |
...
| 情感分析正式页面仍存在 mock 口径 | Phase 2 未处理 UI 数据真实性问题，继续保留 mock 可能误导客户对正式分析能力的判断。 | `docs/exec-plans/active/20260606-brand-monitoring-system-refactor.md`, `web/src/components/*` | Phase 7 接入真实情感事实或明确空状态；正式页面不再用纯 mock 统计代表真实业务数据。 |
