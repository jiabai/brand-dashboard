...
3. 平台管理员从租户列表进入 dashboard 时，URL 必须使用该租户实际拥有的 `tenantKey + jobId + brand`。
4. 没有 job 的租户仍可进入任务状态页，但 dashboard 入口应禁用或不可显示。
...
3. 若最近 job 存在且租户 active，点击“看板”进入 `/dashboard/<tenantKey>/<jobId>?brand=<latestJob.brand>`。
4. 若没有 job，展示“暂无任务”，并保留“任务状态”入口用于后续排查或创建任务。
...
- 前端“看板”入口使用真实 `tenantKey + latestJob.jobId + latestJob.brand`，避免自动默认到竞品品牌。
