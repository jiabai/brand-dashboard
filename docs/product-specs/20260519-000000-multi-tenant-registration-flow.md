...
平台操作员的正式 Web 入口定义在 `docs/product-specs/20260520-010000-platform-operations-console.md`。本文件只描述租户注册、登录和权限主流程；平台运营后台应独立于租户工作台。
...
| 查看本租户仪表板 | 可查看所有 active 租户，只读 | 是 | 是 | 是 | 否 |
| 创建或加载查询任务 | 可代操作 | 是 | 否 | 否 | 否 |
| 查看任务状态 | 本阶段仍需显式租户授权 | 是 | 是 | 是 | 否 |
| 拉取/上报执行任务 | 否 | 否 | 否 | 否 | 是 |
...
- 管理员首次激活入口是公开 `/activate?token=...` 页面，不放在登录后的 `/accounts/:tenantKey` 加入团队页。
- 密码和确认密码不一致时返回 400。
...
- 平台运营人员的正式 Web 后台将从租户工作台拆分到 `/platform/*`，详见平台运营后台规格。
