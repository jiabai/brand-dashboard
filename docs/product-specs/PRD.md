...
35. As a 仪表板用户，I want URL 自动同步当前页面和筛选状态，其中视图、租户、任务由路径承载，时间范围、品牌等筛选条件由查询参数承载，so that 我可以分享带有特定筛选条件的页面链接
...
- 页面视图由 React Router 管理；分析页路径使用 `/:view/:tenantKey/:jobId` 承载视图、租户和任务身份
- 筛选状态由 `DashboardLayout.jsx` 和 `useDashboardParams` 管理，查询参数只保存 `timeframe`, `brand`, `start_date`, `end_date` 等筛选条件
